<div class="flex h-screen bg-gray-100 dark:bg-neutral-900 font-sans antialiased text-gray-800 dark:text-gray-100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($this->filteredProducts as $product)
                    <div
                        ...
                        <div class="p-4">
                            <div
                                class="w-full h-32 bg-gray-200 dark:bg-neutral-700 rounded-lg mb-3 flex items-center justify-center overflow-hidden">
                                @if ($product->imagen_producto)
                                    <img src="{{ asset('storage/' . $product->imagen_producto) }}"
                                        alt="{{ $product->nombre_producto }}" class="object-contain h-32 w-full" />
                                @else
                        ...
                        <div x-data="{ open: false, cantidad: 1 }">
                            ...
                            <div x-show="open"
                                ...
                                <div @click.away="open = false"
                                    class="bg-white dark:bg-neutral-800 rounded-lg shadow-2xl w-full max-w-xs p-6 relative">
                                    ...
                                    <h3 class="text-lg font-bold mb-4 text-gray-900 dark:text-gray-100">Agregar al
                                        carrito</h3>
                                    <div class="mb-4">
                                        <label
                                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cantidad</label>
                                        <input type="number" min="1" :max="{{ $product->stock ?? 1000 }}"
                                    ...
                                    <button @click="$wire.addToCart({{ $product->id }}, cantidad); open = false"
                                        class="w-full py-2 bg-indigo-600 text-white font-bold rounded-lg transition hover:bg-indigo-700">Agregar</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <flux:modal.trigger name="edit-profile">
        <button type="button" class="fixed z-50 bottom-8 right-8 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg p-4 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-blue-500">
